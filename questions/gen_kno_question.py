...
class Gen_Kno_Question():
    ...
    def get_question_no(self):
        ...
        s_count = self.database_handler.get("username", self.app.current_user, "gen_c_s")
        s_count += self.database_handler.get("username", self.app.current_user, "gen_c_f")
        print("TOTAL")
        print(s_count)
    ...
    def check_answer(self):
        ...
        if self.right_ans == self.received_answer:
            ...
            self.database_handler.insert(
                {"piano_c": 0, "piano_l": 0, "piano_r": 0, "gen_c": 1, "chords": 0,
                 "topic": self.topic, "result": 1})

        else:
            self.outcome = "I'm sorry! That was not correct!"
            self.database_handler.increment_database("username", self.app.current_user, "gen_c_f", 1)
            self.database_handler.database_init("users_progress")
            self.mycol = self.database_handler.set_collection(self.app.current_user)
            self.database_handler.insert(
                {"piano_c": 0, "piano_l": 0, "piano_r": 0, "gen_c": 1, "chords": 0,
